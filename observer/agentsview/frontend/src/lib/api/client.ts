...
export interface ListCairnRunsParams {
  session_id?: string;
  project_id?: string;
  intent_id?: string;
  limit?: number;
...
export function listCairnRuns(
  params: ListCairnRunsParams = {},
): Promise<CairnRunsResponse> {
  return fetchJSON(`/cairn/runs${buildQuery({ ...params })}`);
}
