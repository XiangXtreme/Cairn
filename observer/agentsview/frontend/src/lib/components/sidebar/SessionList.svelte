<script lang="ts">
  ...
  let autoExpandedCairnFolders: Set<string> = $state(new Set());
  ...
  let cairnFolderMode = $derived.by(() => {
    const allProjects = [
      ...sessions.projects.map((p) => p.name),
      ...sessions.sessions.map((s) => s.project),
    ].filter(Boolean);
    return allProjects.length > 0 &&
      allProjects.every((p) => p.startsWith("cairn:"));
  });
  ...
  $effect(() => {
    if (!cairnFolderMode) return;
    if (groupMode !== "project") {
      groupMode = "project";
    }
    collapseAll = false;
    const labels = groupSections.map((s) => s.label);
    const next = new Set(manualExpanded);
    const seen = new Set(autoExpandedCairnFolders);
    let changed = false;
    ...
    if (changed) manualExpanded = next;
    if (changed) autoExpandedCairnFolders = seen;
  });
...
>
  ...
  >
    {#each visibleItems as item (item.id)}
      ...
      >
        {#if item.type === "header"}
          {@const headerLabel = cairnFolderMode && item.label.startsWith("cairn:")
            ? item.label.slice("cairn:".length)
            : item.label}
          <button
            class="group-header"
            class:cairn-folder={cairnFolderMode && groupMode === "project"}
          ...
          >
            ...
            <span class="group-name">
              {#if cairnFolderMode && groupMode === "project"}
                <span class="folder-prefix">Cairn</span>
...
<style>
  ...
  .group-header.cairn-folder:hover {
    background: color-mix(in srgb, var(--bg-surface-hover) 88%, var(--accent-blue, #7dcfff) 8%);
  }
