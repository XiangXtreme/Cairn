...
func (s *Server) handleListSessions(
	...
	dbProjectFilter := projectFilter
	if cairnOnlyEnabled() {
		dbProjectFilter = ""
	}

	filter := service.ListFilter{
		Project:          dbProjectFilter,
	...
	page, err = s.filterCairnSessionPage(r.Context(), page, projectFilter)
	if err != nil {
		if handleContextError(w, err) {
			return
		}
...
func (s *Server) handleGetSession(
	...
	id := r.PathValue("id")
	if !s.requireCairnSession(w, r, id) {
		return
	}
	...
	if err := s.rewriteCairnSessionDetail(r.Context(), detail); err != nil {
		if handleContextError(w, err) {
			return
		}
		writeError(w, http.StatusInternalServerError, err.Error())
