...
import { FlaskConical, Plus, RefreshCw, Save, Trash2 } from 'lucide-vue-next';
        ...
        <div class="px-4 py-5 lg:px-6">
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-12">
            ...
            <FormField label="Provider 绑定" class="xl:col-span-3">
              <SelectInput v-model="worker.provider_id" :disabled="worker.provider_supported === false || worker.type === 'mock'">
                ...
                <option v-for="provider in store.providersForWorker(worker)" :key="provider.id" :value="provider.id">
          ...
          <div v-if="worker.type !== 'mock'" class="mt-5 border-t border-slate-200 pt-5">
            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                <div class="text-[11px] font-medium uppercase tracking-[0.12em] text-slate-400">Provider 摘要</div>
                <p class="mt-1 text-xs leading-5 text-slate-500">
                  绑定 Provider 时，模型源与鉴权信息以 Provider 为主；未绑定时使用 Worker 兼容字段。
                </p>
              </div>
              <AppButton size="sm" @click="worker.show_legacy_provider = !worker.show_legacy_provider">
                {{ worker.show_legacy_provider ? '收起兼容字段' : '高级设置' }}
              </AppButton>
            </div>
            ...
            <div v-if="store.selectedProviderForWorker(worker)" class="mt-3 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50/80">
              <div class="grid grid-cols-1 divide-y divide-slate-200 md:grid-cols-[minmax(0,0.9fr)_minmax(0,0.9fr)_minmax(0,1.2fr)] md:divide-x md:divide-y-0">
                <div class="px-4 py-3">
                ...
                <div class="px-4 py-3">
                  <div class="text-[11px] font-medium uppercase tracking-[0.12em] text-slate-400">模型</div>
                  <div class="mt-1 break-all text-sm font-medium text-slate-800">
                    {{ store.selectedProviderForWorker(worker)?.model || '未设置' }}
                  </div>
                </div>
                <div class="px-4 py-3">
                  <div class="text-[11px] font-medium uppercase tracking-[0.12em] text-slate-400">Base URL</div>
                  <div class="mt-1 break-all text-sm font-medium text-slate-800">
            ...
            <div v-else class="mt-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-900">
              当前没有绑定 Provider，将使用下面的兼容字段作为直接配置。
            </div>

            <div
              v-if="worker.show_legacy_provider || !store.selectedProviderForWorker(worker)"
              class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3"
            >
              <FormField label="兼容模型">
                <TextInput v-model="worker.model" />
              </FormField>
              <FormField label="兼容 Base URL">
                <TextInput v-model="worker.base_url" />
              </FormField>
              <FormField label="兼容 API Key / Token" :hint="worker.has_auth_token ? '已存在 key，留空会保持不变。' : '这个 Worker 还没有保存 key。'">
                <TextInput v-model="worker.auth_token" placeholder="留空则保留现有 key" />
              </FormField>
              <template v-if="worker.type === 'pi'">
                <FormField label="兼容 Provider API">
                  <TextInput v-model="worker.provider_api" />
                </FormField>
                <FormField label="兼容上下文窗口">
          ...
          <div class="mt-5 grid grid-cols-1 gap-5 border-t border-slate-200 pt-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
            <div class="min-w-0">
              <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <div class="text-[11px] font-medium uppercase tracking-[0.12em] text-slate-400">额外环境变量</div>
                  <p class="mt-1 text-xs leading-5 text-slate-500">用于补充 provider 专有参数，不覆盖模型、Base URL、Key 这类主字段。</p>
                </div>
                <AppButton size="sm" :icon="Plus" @click="store.addEnv(worker, 'extra_env')">新增变量</AppButton>
              </div>
              <div class="mt-3 space-y-2">
                ...
                <div v-if="store.envEntries(worker.extra_env).length === 0" class="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-3 py-3 text-xs text-slate-400">
                  当前没有额外环境变量。
                </div>
              </div>
            </div>
