...
def run_explore_task(
    ...
    try:
        ...
        LOG.info(
            "starting worker process project=%s intent=%s worker=%s phase=explore_healthcheck timeout=%ss skills=%s",
            project.project.id,
            intent.id,
            worker.name,
            healthcheck_timeout,
...
def _try_conclude_fallback(
    ...
    workspace_name = runtime_manager.ensure_running(project_id)
