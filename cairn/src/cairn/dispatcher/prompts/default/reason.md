...
- If `Open Intents` is empty, propose new intents only when there is a concrete high-value next direction that is not already covered by existing facts or previous intents. If the best next step is simply waiting for an external target or resource to become available again, return empty data instead of proposing a waiting intent.
...
- If recent facts only confirm timeout, unavailability, service hangs, rate limits, or other blocker/waiting conditions, prefer empty data. Do not create intents whose main action is "when the endpoint becomes responsive again", "wait for target recovery", or equivalent.
- Do not create a new intent that repeats a route already represented in facts or intents. Treat variants of the same route as duplicates, for example repeated `gopher`/raw RESP Redis execution attempts, repeated `file://` artifact dumps, repeated webroot PHP writes, or repeated `/proc`/overlay enumeration.
